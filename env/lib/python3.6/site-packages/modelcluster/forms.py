...
class BaseChildFormSet(BaseTransientModelFormSet):
    def __init__(self, data=None, files=None, instance=None, queryset=None, **kwargs):
        ...
        self.rel_name = ForeignObjectRel(
            self.fk, self.fk.remote_field.model, related_name=self.fk.remote_field.related_name).get_accessor_name()

        if queryset is None:
            queryset = getattr(self.instance, self.rel_name).all()

        super(BaseChildFormSet, self).__init__(
            data, files, queryset=queryset, **kwargs)
    ...
    def validate_unique(self):
        ...
        ) and form not in forms_to_delete]
...
class ClusterFormMetaclass(ModelFormMetaclass):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        new_class = super(ClusterFormMetaclass, cls).__new__(
        ...
        opts = new_class._meta = ClusterFormOptions(
        ...
        if opts.model:
            ...
            for rel in get_all_child_relations(opts.model):
                ...
                formset = childformset_factory(
                    opts.model, rel.field.model, **kwargs)
...
class ClusterForm(with_metaclass(ClusterFormMetaclass, ModelForm)):
    def __init__(self, data=None, files=None, instance=None, prefix=None, **kwargs):
        super(ClusterForm, self).__init__(data, files,
        ...
        for rel_name, formset_class in self.__class__.formsets.items():
            ...
            self.formsets[rel_name] = formset_class(
                data, files, instance=instance, prefix=formset_prefix)
    ...
    def is_valid(self):
        ...
        formsets_are_valid = all([formset.is_valid()
    ...
    def save(self, commit=True):
        ...
        instance = super(ClusterForm, self).save(
            commit=(commit and not save_m2m_now))
